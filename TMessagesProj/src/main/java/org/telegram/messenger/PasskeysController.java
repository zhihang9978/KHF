...
public class PasskeysController {
    ...
    public static void create(Context context, int currentAccount, Utilities.Callback2<TL_account.Passkey, String> done) {
        ...
        ConnectionsManager.getInstance(currentAccount).sendRequestTyped(
            ...
            (res, err) -> {
                ...
                requestJson = extractPublicKeyRequestJson(res.options);
                if (requestJson == null) {
                    done.run(null, null);
                    return;
                }
    ...
    private static String extractPublicKeyRequestJson(TLRPC.TL_dataJSON options) {
        if (options == null || options.data == null || options.data.isEmpty()) {
            FileLog.d("initPasskeyLogin options missing publicKey, fallback disabled: options empty");
            return null;
        }
        try {
            final JSONObject obj = new JSONObject(options.data);
            final JSONObject publicKeyObj = obj.optJSONObject("publicKey");
            if (publicKeyObj == null) {
                FileLog.d("initPasskeyLogin options missing publicKey, fallback disabled: publicKey missing");
                return null;
            }
            return publicKeyObj.toString();
        } catch (Exception e) {
            FileLog.d("initPasskeyLogin options missing publicKey, fallback disabled: invalid json");
            return null;
        }
    }
